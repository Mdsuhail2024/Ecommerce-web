...
const Navbar = ({handleOrder}) => {
  ...
  return (
    ...
    <div className='shadow-md bg-white dark:text-white duration:200 relative z-10
    '>
       {/* upper Navbar */}
      <div className="bg-primary/40 py-2 ">
      <div className="container flex justify-between items-center">    {/*mx-auto px-5 sm:px-12*/}
        <div>
         ...
         <div className="flex items-center gap-4 text-black  ">
          ...
          <div className="bg-linear-to-r from-primary to-secondary text-white shadow-2xl rounded-full group relative ">
            <button className='cursor-pointer px-2 hover:px-[10px] py-1 flex items-center gap-2'>
                <span className='text-sm w-0 group-hover:w-[50px] overflow-hidden transition-all duration-300 '>Order</span>
      ...
      <div data-aos="zoom-in" className='flex justify-center text-black'>
       <ul className='hidden md:flex py-4  gap-4'>
        ...
        <li className='group relative  '>
          <a className='px-4 hover:text-primary duration-300 flex items-center gap-1 ' href="#">
            Tranding Products 
            <span className='group-hover:rotate-180 transition-all duration-300'><FaCaretDown /></span></a>
          ...
          <ul className='flex flex-col  gap-2'>
            {TrandingProducts.map((data) => (
              <li key={data.id}>
                <NavLink className='hover:bg-slate-300 w-full inline-block p-1 py-2 rounded-md ' to={data.link}>{data.name}</NavLink>
              </li>
            ))}
          </ul>
        </div>
